...
public class PauseMenu : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        moneyButton = root.Q<Button>("MoneyButton");
        debugPanel = root.Q<VisualElement>("DebugPanel");

        sensitivitySlider.RegisterValueChangedCallback((evt) => {
            sensitivityText.text = "Camera Speed: " + evt.newValue;
            ...
            CameraManager.Instance.sensitivity = CalculateSensitivity(evt.newValue);
        ...
        moneyButton.clicked += () => {
            //CityManager.Instance.Money += 1000000;
        };
        ...
        GameManager.Instance.OnGameStateChanged += (GameState _, GameState state) => {
    ...
    public float CalculateSensitivity(float value) {
        return Map(value, 1, 10, 1, 10);
    }
    public void ToggleDebugPanel() {
        if (debugPanel.style.display == DisplayStyle.None) {
